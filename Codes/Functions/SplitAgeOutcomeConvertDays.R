...
SplitAgeOutcomeAndConverToDays <- function(age_outcome)
{
  ...
  if (grepl("year", split_unit[[1]][2], ignore.case = TRUE))
  {
    return (as.numeric(split_unit[[1]][1]) * 365)
  } 
  else if (grepl("month", split_unit[[1]][2], ignore.case = TRUE))
  { 
    return (as.numeric(split_unit[[1]][1]) * 30)
  } 
  else if (grepl("week", split_unit[[1]][2], ignore.case = TRUE))
  {
    return (as.numeric(split_unit[[1]][1]) * 7)
  } 
  else if (grepl("day", split_unit[[1]][2], ignore.case = TRUE))
  {
